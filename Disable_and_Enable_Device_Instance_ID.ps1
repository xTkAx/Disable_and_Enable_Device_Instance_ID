...
Function GetAllEnumDevices #Done
{
	return pnputil /enum-devices
}


<# This function will parse the string array from pnputil /enum-devices into List[Device] sorted by DeviceDescription #> #Done
function ParseAndSortAllEnumDevicesData([String[]] $EnumDevicesListing)
{
	...
	{
        # Add $CurrentDevice to $NewDevices if the line is empty (indicating a break in device listins) and $CurrentDevice has an Instance ID:
...
<# This function will search the Device List for a an InstanceID to find a direct TargetDevice #> #Done
...
<# This will search all List[Device] fields for a query and return a list of found Devices #> #Done
...
while($Global:RunProgram)
{
    ...
    if ($Global:UserQuery -ne "")
    {
        ...
        if ($Global:TargetDevice -eq $Null)
        {
            ...
            {
                Write-Host "Found Target Device."
